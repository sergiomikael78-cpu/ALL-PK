...
(function () {
  ...
  function saveTemplates(pushToCloud = true) {
    try {
      ...
      updateCategoryCounts();
      if (window.MobileHub && typeof window.MobileHub.updateStats === 'function') {
        window.MobileHub.updateStats();
      }
      if (pushToCloud) {
        syncChangeToCloud();
      }
  ...
  function createCardHtml(item) {
    ...
    const hasTrailingSpace = typeof item.trigger === 'string' && item.trigger.endsWith(' ');
    let rawTriggerToDisplay = hasTrailingSpace ? item.trigger.slice(0, -1) : (item.trigger || '');
    let displayTrigger = escapeHtml(rawTriggerToDisplay);
    ...
    if (hasTrailingSpace) {
      displayTrigger += ` <span class="badge-space-hint" style="background: rgba(0,242,254,0.15); border: 1px solid rgba(0,242,254,0.4); border-radius: 4px; padding: 1px 5px; font-size: 10px; color: #00f2fe; margin-left: 2px;">␣ Spasi</span>`;
    }
  ...
  function handleSaveTemplate(e) {
    ...
    const rawTrigger = elements.inputTrigger.value || '';
    // Bersihkan spasi di awal saja, pertahankan spasi di akhir jika sengaja diinput (misal "/q ")
    const trigger = rawTrigger.replace(/^\s+/, '');
